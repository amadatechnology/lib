...
app.post('/create-customer', async (req, res) => {
  try {
    ...
    // Call the function from stripeFunctions to create a customer
    const customer = await stripeFunctions.createCustomer(email, payment_method);
...
app.post('/create-subscription', async (req, res) => {
  try {
    ...
    // Call the function from stripeFunctions to create a subscription
    const subscription = await stripeFunctions.createSubscription(customer_id, price_id);
...
app.post('/upgrade-subscription', async (req, res) => {
  try {
    ...
    // Call the function from stripeFunctions to upgrade a subscription
    const updatedSubscription = await stripeFunctions.upgradeSubscription(subscription_id, new_price_id);
    res.json({ updated_subscription: updatedSubscription });
  } catch (error) {
...
app.post('/create-checkout-session', async (req, res) => {
  try {
    ...
    // Retrieve or create the relevant Price ID based on the selected tier
    let priceId;

    switch (tier) {
      case 'free':
        priceId = 'price_1OntgjAtxVrFPacKIkTqR5w9'; // Replace with the actual free tier Price ID
        break;
      case 'silver':
        priceId = 'price_1OnrLnAtxVrFPacKeCaNKQUk'; // Replace with the actual silver tier Price ID
        break;
      case 'gold':
        priceId = 'price_1OnrHnAtxVrFPacKN0qcJNoI'; // Replace with the actual gold tier Price ID
        break;
      default:
...
// Update selected tier endpoint
app.post('/updateSelectedTier', async (req, res) => {
  const { userId, selectedTier } = req.body;

  try {
    // Update the user's selected tier in the database
